...
`/admin/analytics` → **제출 링크**에서 회사·지원 직무·제출일을 입력하고 링크를 발급한다. 목록에 링크를 항상 표시하며 **링크 복사**로 같은 주소를 다시 사용할 수 있다. 수정·비활성화·재활성화해도 링크 주소는 유지된다.

**방문 기록**에서 기간·회사 링크·기기로 조회하고 상세 보기에서 페이지, 섹션, 프로젝트별 활성 체류와 클릭 순서를 확인한다. 분석 기능이 켜진 공개 페이지에서는 별도 허용/거절 팝업 없이 기록이 자동으로 쌓인다. 링크 방문은 해당 회사 담당자의 신원이나 독해를 증명하지 않으며, 추적 차단·자동 미리보기·공유에 따라 누락 또는 오인 가능성이 있다.

세션은 문서/탭 단위다. 새로고침과 새 탭은 새 세션이며, 분석 기능 활성화 후 생성한 무작위 browserId로 브라우저 수를 추정한다. 30분 활동 없음 또는 24시간 경과 뒤 새 세션을 만든다. 전체 활성 시간은 page 이벤트만 합산하며 섹션·모달 시간을 다시 더하지 않는다. 기기·브라우저 계열은 User-Agent에서 축약하고 원문은 분석 테이블에 저장하지 않는다.
...
`202609070003_analytics_saved_links.sql`은 관리자 재조회를 위한 `share_token`을 추가한다. 새 링크는 발급한 원문을 저장한다. 이전에 해시만 저장했던 링크는 같은 link ID를 가리키는 고정 별칭을 한 번 생성한다. 기존 `token_hash`를 변경하지 않으므로 이미 제출한 주소도 계속 같은 방문 기록에 연결된다. 목록을 조회할 때마다 새 링크를 발급하지 않는다. 두 주소 모두 동일한 비활성화 설정을 따른다.

토큰 원문은 기존 관리자 인증과 RLS를 통과한 링크 관리 API에서만 조회한다. 공개 config/방문 조회 데이터에는 추가하지 않는다. 운영 적용 순서는 migration 003 후 앱 배포이며 이전 앱도 기존 token_hash로 계속 동작한다.

## 프로젝트별 관심도

방문 기록 화면의 **프로젝트별 관심도**는 같은 기간·제출 링크·기기·자동화/테스트 필터를 적용한다. 프로젝트를 연 이벤트 또는 프로젝트 노출 이벤트가 있는 세션을 중복 없이 세어 열람 방문 수로 표시한다. 모달/상세 페이지 열람은 `project_open` 횟수이며 반복 열기는 누적된다. 활성 시간은 프로젝트 영역의 `exposure_delta`만 합산하고 페이지 전체 시간은 더하지 않는다. 방문당 활성 시간은 이 합계를 열람 세션 수로 나눈 값이다.

프로젝트에 연결된 GitHub/데모 클릭은 각각 누적한다. 열람 없이 클릭만 수신된 경우 클릭은 표시하되 열람 방문을 임의로 늘리지 않는다. 열람 방문 수, 활성 시간, 열람 횟수 순으로 정렬하며 방문 목록의 페이지를 넘겨도 프로젝트 전체 집계는 유지한다. 삭제된 프로젝트의 과거 기록은 프로젝트 ID로 표시한다. 기존 이벤트를 집계하므로 별도 DB 마이그레이션이나 과거 데이터 재수집은 필요하지 않다.
